...
public class SBIVerificaltionResponse {
	...
	public String toString() {
		return "SBIVerificaltionResponse [bank_ref_no=" + bank_ref_no + ", amount=" + amount + ", ref_no=" + ref_no
				+ ", status=" + status + ", status_description=" + status_description + ", checkSum=" + checkSum + "]";
	}
	
	public SBIVerificaltionResponse(String str) {
		...
		this.status_description = arr[4].split("=").length <=1 ? "": arr[4].split("=")[1];
		this.checkSum = arr[5].split("=").length <=1 ? "": arr[5].split("=")[1];
